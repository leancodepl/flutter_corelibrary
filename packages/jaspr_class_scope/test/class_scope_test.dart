...
const heroScope = ClassScope('Hero', '0dc8kc');
const navBarScope = ClassScope('NavBar', 'lffm33');
...
void main() {
  ...
  group('ClassScope', () {
    test('renders a class as local-suffix', () {
      expect(heroScope('grid').name, 'grid-0dc8kc');
      expect(heroScope('grid').selector, '.grid-0dc8kc');
    });
    ...
    test('throws when two components take one suffix', () {
      expect(heroScope('grid').name, 'grid-0dc8kc');
      expect(
        () => const ClassScope('OtherHero', '0dc8kc')('grid').name,
        throwsA(
          isA<StateError>().having(
            (e) => e.message,
            'message',
            allOf(contains('Hero'), contains('OtherHero'), contains('-0dc8kc')),
          ),
        ),
      );
    });
  ...
  group('ClassName', () {
    ...
    test('combines classes onto one element', () {
      ...
      expect(combined.name, 'button-0dc8kc primary');
      expect(combined.selector, '.button-0dc8kc.primary');
    });
    ...
    test('is a value: equal when it renders the same', () {
      expect(heroScope('grid'), const ClassScope('Hero', '0dc8kc')('grid'));
    ...
    test('stringifies to the name it renders', () {
      expect('${heroScope('grid')}', 'grid-0dc8kc');
